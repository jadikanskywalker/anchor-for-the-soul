    ...
    for (i = 0; i < transformedText.length; i++) {
      if (transformedText[i] == "/") {
        ...
        newText =
          ...
          tagType +
          ">" +
          transformedText.substring(i + 3)
        transformedText = newText.slice()
        i = i + 2 + tagLength
        for (let j = i; j < transformedText.length; j++) {
          if (transformedText[j] == "/") {
            newText =
              transformedText.substring(0, j) +
              "</" +
              tagType +
              ">" +
              transformedText.substring(j + 1)
            transformedText = newText.slice()
            i = j + 3 + tagLength
            break
          }
